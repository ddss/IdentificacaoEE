...
nvars = nRetas; % posicoes dos pontos de cortes (nRetas - 1) + numero de pontosAtivos
% Limite inferior (pontos 1 e 2 sao automaticos -> vide estimacao)
LB = 3*ones(1,nRetas);

% Limite superior
UB = [ones(1,nRetas-1)*length(serie),nRetas-1];
...
options= gaoptimset('UseParallel','always','TolFun',1e-12,'TolCon',1e-12,...
    'PopulationSize',500,'Generations',1000);
...
fprintf(fileID,'Estados Estacionarios identificados (numero da reta) \n');
for pos = 1:length(fval)
